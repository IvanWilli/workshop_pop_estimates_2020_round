...
census_LA %>% 
     ...
     ggplot(aes(Año, País)) + 
     geom_point() +
     ggtitle("Población por edad y sexo de censos inlcuidos") +
     theme_minimal()
```

Respecto a las defunciones y nacimientos, los data.frame `deaths_LA` y `births_LA` contienen información de eventos por sexo (del recién nacido en el primer caso) y edad (de madres en el primer caso). Por ejemplo Panama en 2015:

```{r}
deaths_LA %>% 
     filter(País == "Panama", Año == 2015) %>% 
     head()
```
...
Si quieres ver en profundidad su obtención puedes ver el código `get_data.R`.

## Población

### Evolución censal

Sigamos con el caso de Panamá, aprovechando la publicación reciente de sus datos censales. La dinámica demográfica de este país estuvo muy relacionada con el desarrollo del canal, inlfuenciando el nivel de desarrollo económico y la condición de país atractor para la migración [@INEC2016].
...
Cabe aclarar que no nos adentraremos en detalle sobre la evolución y sitaución actual demográfica de Panamá, ni daremos estimaciones más acabadas que las publicadas por el INEC (por ejemplo solo tomamos población censada, no ajustada), sino que nos servirá para trabajar en R disintas aplicaciones.

### Edad y sexo

Visualizemos su distribución por edad y sexo. Lo primero que se observa es un proceso de envejecimiento, con angostamiento en la base, y resalta a simple vista una declaración por edad más precisa con el tiempo que luego mediremos más en detalle.

```{r}
panama %>%
     filter(Año != 1970, Edad < 100) %>% # el censo de 1970 se encuentra agrupado por edad quinquenal
...
El último censo nos muestra una base más pequeña, sugiriendo una evolución intercensal de nacimientos decreciente.
...
El índice de masculinidad permite ver el proceso de feminización visto en la tabla resumen, sobre todo en edades activas. El IM en censos previos a 1990 muestra una gran variabilidad, por lo que utilizamos la función `geom_smooth` de `ggplot` para tener idea de su patrón. Aquí puede tener una gran influencia el cambio en el tipo de censo, donde el último relaizado es el primero *de-jure* (o *de derecho*), enfocándose en captar la población residente.
...
En términos de análisis intercensal por cohorte, en ausencia de importantes eventos de impacto demográfico (flujos migratorios, guerra, desplazamientos, etc.), y en ausencia tambien de omisión diferencial por censo, se esperaría ver que el ratio por cohorte refleje el efecto de la mortalidad [@Siegel2004]. Su cálculo:

$$
R_x(t, t+n) = \frac{P_{x+n}(t+n)}{P_{x}(t)}
$$

El ratio se puede calcular por edad simple o por grupos quinquenales para evitar fluctuaciones. En los dos últimos períodos intercensales tenemos distintos intervalos, pero igual nos permite conocer algo. Asumamos por ahora que la distancia de 10 y 13 años respectivamente para los dos últimos períodos intercensales, y recorramos el código:
...
En algunos casos, una visión por grupos quineuanles de edad puede ser más ilustrativa visualmente, removiendo la aleatoriedad propia de la desagregación por edad simple. En base a estos primeros diagnósticos visuales podemos identificar algunos aspectos que podemos analizar o ajustar mediante técnicas demográficas específicas.

### Edad y sexo ignorada
...
# peso en el total censado
sum(panama_2023$Pop[is.na(panama_2023$Edad)])/sum(panama_2023$Pop)
```
...
# chequiemos que el total se respete
...
En el caso de que se tenga más de una variable con desconocido, típicamente sexo, el paso previo debe ser repetido: una opción es primero distribuir el ignorado de edad dentro de cada sexo, y luego el sexo. Otras técnicas pueden ser la imputación mediante métodos estadísticos a nivel del microdato.

### Atracción de edad y suavizamiento

La distribución por edad puede mostrar algunos picos que podrían atribuirse a sesgos en la declaración, por atracción hacia ciertos dígitos, o hacia edades más avanzadas. Existen varios indicadores de actracción [implementados en DemoTools](https://timriffe.github.io/DemoTools/articles/Age-heaping_quality_with_Demotools.html) para su diagnóstico. Por ejemplo para el año 2023, en hombres, solo para edades entre 10 y 65:
...
data.frame(Indicador = c("Whipple", "Myers", "Bachi"),
           Valor = c(whipple_panama2023_hombre, myers_panama2023_hombre, bachi_panama2023_hombre)) %>% kbl(caption = "Indicadores de atracción de edades. Panamá 2023") %>% kable_classic(full_width = F)
```

Respecto al rango de valores "aceptables", Whipple con resultados para dígitos 0 y 5 va de 1 (sin heaping) a 5 (concentración total), Myers mide la atracción para todos los dígitos y va de 0 (sin heaping) a 90 (mayor atracción), siendo Bachi muy similar a este último. Como se ve, la declaración sepuede considerar muy buena. También se ve una alta correlación entre índices.
Pero quizás convenga también verlo desde una perspectiva histórica: el reciente censo presenta una declaración mucho más precisa que los anteriores.
...
panama_heaping_index <- panama %>% 
     ...
     map_df(function(census){
          # print(paste0(unique(census$Anio), unique(census$Sexo)))
          # census <- panama %>% filter(!is.na(Edad), `Año` == 1950, Sexo == "Hombre")
...
En el protocolo de WPP-NU [@WPP_Method2022], la División de Población consideró el índice de Bachi para los casos de edad simple (o el Puntuación de la proporción de edades para recuentos por grupo de edad de cinco años), junto con el nivel medio de educación completado por los adultos de la población, para determinar el grado de suavizamiento sobre la edad a aplicar mediante una media móvil.

La población por edad en el último censo tiene una distribución.
...
LO ideal sería contar con una encuesta post-censal que permita inferir la distribución por edad más acorde. Cuando no es posible, distintas opciones estan disponibles para suavizar. Aquí vemos algunas implementadas en `DemoTools`: 

```{r}
graduación <- panama_2023 %>% 
     split(.$Sexo) %>% 
     map_df(function(panama_2023_sexo){
          # panama_2023_sexo <- panama_2023 %>% filter(Sexo == "Mujer")
...
Siempre es importante checkear totales y distribución por grandes grupos de edad, aunque algunos métodos lo garanticen (`Sprague`):
...
En el caso de WPP-NU [@WPP_Method2022], las evaluaciones de acumulación de edad y el suavizado se realizaron por separado para las edades de niños y adultos y el  las series resultantes se sumaron al postajuste. Cuando la serie de datos del censo se agrupó por grupos de edad de cinco años, la serie ajustada y suavizada finalmente se graduó hasta un solo año de edad utilizando un spline monótono. Cuando fue necesario, los recuentos se redistribuyeron al grupo de edad indefinido de 105 años o más utilizando la función OPAG del Paquete DemoTools R, que aplicó la tabla de vida estimada para el país y el año del censo como un estado estable.
estándar.

### Edades iniciales

Vimos en la pirámide que las edades iniciales sugieren una reciente baja de la natalidad, sin embargo el descenso de la natalidad parece comienzar en 2019. Evaluaremos la población en edades menores a 10 sobreviviendo los nacimientos de esas cohortes. 
...
lexis_grid(year_start = 2020, year_end = 2023, age_start = 0, age_end = 2) %>% 
     ...
     ggtitle("Triángulo superior e inferior de las defunciones de edad 0 en el año 2021. Diagrama teórico") +
     annotate("text", x = as.Date("2021-04-01"), y = .75, label = "a(0)", col = "white") +
     annotate("text", x = as.Date("2021-10-01"), y = .25, label = "1-a(0)", col = "black")
```


Para obtener $a_0$ existen muchas opciones. `DemoTools` pone a disposición las opciones de Coale-Demeny, la utilizada por el grupo HMD a traves de la función `lt_rule_1a0`, o Keyfitz-Flieger (Watcher, 2006) que toman como argumento la tasa de mortalidad infantil (definicda como $D_0(t)/B(t)$) [@Preston2000, @HMDProtocol, @Wachter2006]. Tomemos el promedio del período como referencia:
...
pop_0a9_2023_mujeres$plot_lexis +
     ggtitle("Evolución por cohorte según eventos observados. Panamá") 
```
...
panama_2023 <- read.csv("census_LA.csv") %>% 
     filter(Pais == "Panama", Anio == 2023)

ggplot() +
     geom_line(data = panama_2023 %>% filter(Edad < 20), aes(Edad, Pop))+
...
En términos cuantitativos, la diferencia es de alrededor del 5\%, pero como el gráfico muestra la diferencia no es igual entre edades.

```{r}
censo <- panama_2023 %>% filter(Edad < 10) %>% summarise(Pop = sum(Pop)) %>% pull(Pop)
lexis <- panama_lexis_pop2023 %>% filter(age < 10) %>% summarise(Pop = sum(pop)) %>% pull(Pop)
data.frame(Censo = censo, Lexis = lexis, Dif_Abs = censo - lexis, Dif_Rel = (censo - lexis)/censo)
```


### Edad finales

Método directo e indirecto de Generación extintas

## Mortalidad

### Construcción y graduación

Obtengamos la tabla de mortalidad de Panamá para el año 2022. Para eso, requerimos una estimación de la población expuesta de mujeres, por lo que haremos una interpolación por grupo de edad para correr un desde la fecha de referencia censal ("01/02/2023") hacia mitad de año de 2022.
...
panama_mx5_plot +
     ...
     ggtitle("Tasa de mortalidad por edades agrupadas a partir de datos censales y defunciones observadas, y extensión hacia edades finales. Panamá 2022")
```

Como se ve, ambas estimaciones se diferencian a partir de la edad 80, ya que desde la edad 50 fueronse utilizó para su ajuste. Por suerte,esta funcionalidad esta ya presente en las funciones de DemoTools, para crear una tabla de mortalidad. Seleccionemos `Makeham`.
...
Tambien podemos graduar hacia edades simples ya que la función `lt_abridged2single`:
...
panama_mx5_plot +
     ...
     ggtitle("Tasa de mortalidad por edades agrupadas a partir de datos censales y defunciones observadas, y graduación por edad simple. Panamá 2022")
```
...
Existen situaciones donde nuestra confianza en las estadísticas vitales no es tan buena, y solo contamos con indicadores resumen como $q_0$, ${}_{5}q_0$, $e_0$ o ${}_{45}q_{15}$, a partir de estimaciones indirectas. O nos interesa saber qué patrón por edad seguiría la mortalidad de nuestra población bajo una de las familias de tablas de mortalidad [@UN]. Para esto tenemos dos funciones implmenentadas por Sara Hertog, adaptando funciones d eun software llamado MortPack (disponible online [aquí](https://teams.microsoft.com/l/message/19:8f377ceb-a5da-44d2-8deb-0d0994fa73bd_cab0a12e-9497-49ac-8b55-89f03bdb3c43@unq.gbl.spaces/1700503389333?context=%7B%22contextType%22%3A%22chat%22%7D) para aquellos más amigos de excel).

Por ejemplo bajo la familia Oeste de Coale Demeny, teniendo como insumo solo $e_0$, esta función encuentra la interpolación de tasas que cumple con el valor dado:
```{r}
...
Pero tambien podemos tener dos valores, y buscar el nivel que mejor ajuste. En este caso contamos con ${}_1q_0$ y ${}_45q_{15}$:
...
panama_2022_mlts %>% 
     ggplot() +
     geom_line(aes(Age, nMx, col = MLT)) + 
     scale_y_log10()+
     scale_x_continuous(breaks = seq(0,100,10), labels =  seq(0,100,10), limits = c(0,100)) +
     ggtitle("Tasa de mortalidad por edades simples a partir de datos censales y defunciones observadas, y tablas modelo CD familia West. Panamá 2022") +
     theme_bw()
```

¿Qué conclusiones podemos obtener?

Otra opción interesante es la de utilizar el modelo log-quad [@Wilmoth2012], para saber, bajo el patrón de experiencia histórica HMD, cuál serían las tasas por edad que corresponderían a nuestro valor de ${}_1q_0$. Esta función esta implemnetada en paquetes como `MortCast` o `MortalityEstimate`, pero también en [DemoTools](https://timriffe.github.io/DemoTools/reference/lt_model_lq.html)!

## Fecundidad

### Graduación

Calculemos las tasas abreviadas de fecunidad por edad para Panamá en el año 2022. Para eso, requerimos una estimación de la población expuesta de mujeres, por lo que haremos una interpolación por grupo de edad para correr un desde la fecha de referencia censal ("01/02/2023") hacia mitad de año de 2022.
```{r}
# estimar exposición en 2022
panama_pop5_2010_2023 <- panama %>%
     filter(País == "Panama", Año %in% c(2010, 2023), !is.na(Edad)) %>% 
     # convertir la fecha a número con dec.date (DemoTools)
     mutate(Date = dec.date(dmy(Fecha)),
            Edad = pmin(trunc(Edad/5)*5, 100)) %>% 
     summarise(Pop = sum(Pop), .by = c(Date, Sexo, Edad))
panama_exposures5_2022 <- bind_rows(
     interp_tidy(panama_pop5_2010_2023 %>% filter(Sexo == "Mujer"), Edad, Date, Pop, 2022.5) %>% 
          mutate(Sexo = "Mujer"),
     interp_tidy(panama_pop5_2010_2023 %>% filter(Sexo == "Hombre"), Edad, Date, Pop, 2022.5) %>% 
          mutate(Sexo = "Hombre"))
...
panama_asfr5 %>% 
     ...
     geom_step() +
     ggtitle("Tasas de fecundidad por edades quinquenales a partir de datos censales y nacimientos observados. Panama 2022") +
     theme_bw()
```

Aquí trataremos los métodos de Splines calibrados de [@Schmertmann2014] (código disponible [aquí](https://schmert.net/calibrated-spline/REBEP/)) y Beers (misma método utilizado en secciones previas). Diversas pruebas hechas por el grupo de [HFD](https://www.humanfertility.org/File/GetDocument/Docs/paa/Grigoriev_Jdanov.pdf) muestran que en términos de flexibilidad y ajuste, el orden decreciente sugerido es CS, Spline y Beers. Intentemos  qué resulta para nuestro caso: 
...
asfr_graduation <- data.frame(
     Edad = rep(15:49, 2),
     Method = c(rep("Beers", 35), rep("CS", 35)),
     asfr = c(asfr_beers, asfr_cs))
asfr_graduation %>% 
     ggplot(aes(Edad, asfr)) +
     geom_line(aes(col = Method)) +
     geom_point(data = panama_asfr5, aes(Edad_media, asfr)) +
     ggtitle("Tasas de fecundidad por edades quinquenales y graduación por edades simples. Panamá 2022") +
     theme_bw()
```
...
Si quisiéramos visualizar la matriz, tendríamos lo siguiente, donde los marginales estan coloreados:
...
```{r,eval = F}
library(DemoTools)
DemoTools::mig_un_fam(NM = 5000, family = "Family")
# DemoTools::mig_estimate_rc()
```


## Actividad

a) Selecciona un país que te interese y obtén el gráfico de pirámides censales. ¿Qué le llama más la atención?

a) Obtén la evolución temporal de algún indicador de atracción de edad.

a) Para algún año disponible, obtener las tasas de mortalidad por edad y contruir una tabla de mortalidad mediante DemoTools. Suavizarlas por edad simple si es necesario y comparar los resultados.

Por último, si sos tan amable, puedes contestar esta breve encuesta sobre el espacio? Gracias!
